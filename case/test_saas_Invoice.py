...
def _create_invoice_order(client: httpx.Client, token_id: str, index: int) -> str:
    with step(f"创建第{index}笔订单"):
        ...
        attach_text(f"第{index}笔订单号", order_id)
        return order_id
...
def _apply_order_invoice(client: httpx.Client, order_ids: Any, token_id: str) -> str:
    with step("申请开票"):
        invoice_id, apply_resp = apply_invoice_for_order(
            client, order_ids, token_id=token_id, return_response=True
        )
        attach_text("开票号", invoice_id)
        attach_json("开票接口响应", apply_resp)
...
def _apply_invoice_payload(
    ...
    *,
    step_title: str,
    payload_attachment_name: Optional[str] = None,
) -> str:
    with step(step_title):
        if payload_attachment_name:
            attach_json(payload_attachment_name, payload)
...
def _refresh_invoice(client: httpx.Client, invoice_id: str, token_id: str) -> None:
    with step("刷新开票状态"):
        refresh_resp = refresh_invoice_status(
            client, invoice_id, token_id=token_id)
        attach_json("刷新开票状态接口响应", refresh_resp)
...
def _query_invoice(
    ...
    with step("查询开票状态"):
        ...
        attach_text("开票状态", invoice_status)
        attach_json("查询开票状态接口响应", detail_resp)
        assert_detail_response(
            detail_resp, invoice_id, order_ids, invoice_status)
        return invoice_status


def _verify_invoice_status(
    client: httpx.Client,
    invoice_id: str,
    order_ids: Any,
    token_id: str,
) -> str:
    _refresh_invoice(client, invoice_id, token_id)
    return _query_invoice(client, invoice_id, order_ids, token_id)


def _red_punch_and_assert(client: httpx.Client, invoice_id: str, token_id: str) -> None:
    with step("红冲开票"):
        red_resp = red_punch_invoice(client, invoice_id, token_id=token_id)
        attach_json("红冲接口响应", red_resp)
        assert_red_punch_response(red_resp)
        logger.info("红冲完成，开票号=%s", invoice_id)


def _execute_raw_invoice_request(
    client: httpx.Client,
    payload: Dict[str, Any],
    token_id: str,
    step_title: str,
    ...
    with step(step_title):
        ...
        attach_text("响应状态码", resp.status_code)
        attach_json("响应内容", response_json)
        logger.info("接口响应状态码: %s", resp.status_code)
        logger.info("接口响应内容: %s", response_json)
...
class TestSaasInvoice:
    ...
    def test_apply_invoice(self, client, create_invoice_order, access_token):
        ...
        invoice_id = _apply_order_invoice(client, order_id, token_id)
    ...
    def test_merge_invoice(self, client, create_merge_invoice_orders, access_token):
        ...
        invoice_id = _apply_invoice_payload(
            client,
            payload,
            order_ids,
            token_id,
            step_title="合并开票",
            payload_attachment_name="合并开票请求参数",
        )
        _verify_invoice_status(client, invoice_id, order_ids, token_id)
        _red_punch_and_assert(client, invoice_id, token_id)
    ...
    def test_merge_invoice_empty_order_id_return_null_pointer(self, client, access_token):
        ...
        with step("构建 orderId 为空的请求参数"):
            payload = build_empty_order_id_payload(token_id)
            attach_json("开票请求参数", payload)
            logger.info("构建 orderId 为空的合并开票请求参数: %s", payload)

        response_json = _execute_raw_invoice_request(
            client, payload, token_id, "发送合并开票请求")

        with step("校验返回订单不存在错误"):
    ...
    def test_merge_invoice_amount_mismatch(self, access_token):
        ...
        with step("构建合并开票请求"):
            ...
            attach_text("异常信息", str(exc_info.value))
    ...
    def test_apply_invoice_invalid_token(self, client, create_invoice_order):
        ...
        with step("准备无效令牌请求"):
            attach_json("开票请求参数", payload)
            attach_text("使用的无效令牌", invalid_token)
            logger.info("使用固定过期令牌申请开票%s", payload)

        response_json = _execute_raw_invoice_request(
            client, payload, invalid_token, "发送开票请求")

        with step("校验返回登录已失效错误"):
    ...
    def test_apply_invoice_exceed_amount(self, client, create_invoice_order, access_token):
        ...
        response_json = _execute_raw_invoice_request(
            client, payload, token_id, "发送开票请求")

        with step("校验返回开票金额超限错误"):
    ...
    def test_none_order_invoice(self, client, access_token):
        ...
        with step("构建无订单开票请求参数"):
            payload = build_none_order_invoice_payload(token_id)
            attach_json("无订单开票请求参数", payload)
